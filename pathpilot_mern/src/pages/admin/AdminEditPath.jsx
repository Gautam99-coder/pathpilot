  ...
  useEffect(() => {
    const fetchPath = async () => {
      try {
        ...
        if (res.ok) {
          const currentPath = await res.json();
          setRoadmapTitle(currentPath.title || "");
          setDescription(currentPath.description || "");
          setCardImage(currentPath.image || null);
          if (currentPath.phases && currentPath.phases.length > 0) {
             const loadedPhases = currentPath.phases.map((p, idx) => ({
                id: idx + 1,
                title: p.title,
                content: p.content,
                link: p.videoUrl || "",
                file: null,
                quiz: p.quizzes.map((q, qIdx) => ({
                   question: q.question,
             ...
             setNextId(loadedPhases.length + 1);
          }
        }
    ...
    <div className="flex h-screen bg-[#f8f9fc] font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-white px-10 py-5 flex justify-between items-center border-b">
          <h1 className="text-xl font-bold">EDIT Career Path</h1>
          <button
